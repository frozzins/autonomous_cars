...
The hypothesis was tested by comparing number of accidents in phase 1 and phase 2 in both groups of participants. In order to ensure validity of the results an attempt to identify all factors influencing number of collisions was made. As a result the following list was created:

\begin{itemize}
  \item Density of the vehicles (number of cars per track length)
  \item Participants improving their skills in controlling the vehicle in consecutive phases.
  \item Initial placement of vehicles
  \item Length of each phase
  \item Non-realistic decelerations of autonomous cars.
\end{itemize}


The algorithm governing the behaviour of autonomous cars was based on a model that is equipped with emergency breaking routine. If necessary, it allows the car to decelerate at rate that would be impossible in real world. In trying to find the impact of autonomous cars on the traffic this kind of behaviour could distort the conclusions. It is however hoped that its influence can be accounted for or mitigated. 


Number of accidents was be compared between first and second phase for both groups. 

...
\subsubsection{Hypothesis: Autonomous cars reduced number of accidents}

Based on the observations from previous section it can be seen that autonomous vehicles were almost never responsible for collisions. However, there were numerous collisions caused by other traffic participants involving autonomous cars. Based on these premises the following hypothesis will be tested:

\textbf{\textit{"Replacing part of traditional drivers with autonomous cars reduces frequency of all kinds of accidents"}}

\paragraph{Method of analysis}

The hypothesis was tested by comparing number of accidents in phase 1 and phase 2 in both groups of participants. In order to ensure validity of the results an attempt to identify all factors influencing number of collisions was made. As a result a list presented in Table~\ref{table:hypothesis1} was created.
...
6         &                                                                                      &                                                                                                                                                      \\ \hline
\end{tabular}
\caption{Factors influencing the number of accidents.}
\label{table:hypothesis1}
\end{table}



bla bla..however large number of collisions with autonomous cars in third scenario karze przypuszczac that alogorithm governing autonomous cars does not perfom that well on more complicated map.


According to \citep{vangi2007evaluation} deceleration for Renault Clio(R sing) in good driving conditions from the speed 15m-s reaches maximum of 12m=s2. Although this value might be greater for cars with better brakes it was accepted as a border value. Any situation when deceleration was greater that 12m-s2 was analysed seperately to classify it as potential collision. 





Number of accidents was be compared between first and second phase for both groups. In first group there were 11 collision in first scenario and 5 collisions in second scenario. there were 3 situation where deceleration of autonomous vehicle reached values below 12m/s2. None of them was classified as potential collision. 

For second group ..same thing...



In order to confirm or deny the hypothesis

However, such sudden deceleration did not happen often. 

his kind of routine 

\citep{vangi2007evaluation} - max deceleration will ocilate around 12-16 probably depending on the type of vehicle and speed.
...
Factors influencing number of collisions in particular group include:
